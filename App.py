...
def run():
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                if reco_field == 'Unknown':
                    st.warning("⚠️ Could not detect field from resume skills")
                ...
                email = resume_data.get('email') or "unknown@gmail.com"
        ...
        if st.button('Login'):
            if ad_user == 'Affan' and ad_password == 'Affan123':
                st.success("Welcome Mohamed Affan!")
                # Display Data
                ...
                plot_data = plot_data.applymap(
                lambda x: x.decode() if isinstance(x, bytes) else x
                )

               # 🔥 Convert BLOB (bytes) → string BEFORE plotting
